...
export default function middleware(request: NextRequest) {
  ...
  if (isProtectedRoute && hasOAuth && !hasEmail) {
    const sync = new URL("/api/auth/sync-session", request.url);
    sync.searchParams.set("next", sanitizeAuthRedirect(pathname));
  ...
  if (onAuthRoute && !bypassAuthRedirect) {
    ...
    // Do not auto-redirect /login → sync-session (blocks starting a fresh Google OAuth flow).

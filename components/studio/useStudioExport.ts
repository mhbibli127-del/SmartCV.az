...
async function collectPaperElements(editor: CanvasEditorHandle): Promise<HTMLElement[]> {
    ...
    async (format: StudioExportFormat) => {
      ...
      try {
        ...
        if (format === "pdf") {
          ...
          const papers = await collectPaperElements(editor);

          if (papers.length === 0) throw new Error("Export target not found");



          const result = await exportPaperPdf(papers, title);
        ...
        await editor.preparePage(useEditorStore.getState().activePage);

        const paper = editor.getPaperElement();

        if (!paper) throw new Error("Export target not found");



        const result = await exportPaperImage(paper, format, title);
